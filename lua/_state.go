...
import (
	"context"
	"fmt"
	"github.com/vela-security/vela-public/lua/parse"
	"io"
	"math"
	"os"
	"runtime"
...
// newCallFrameStack allocates a new stack for a lua co, which will auto grow up to a max size of at least maxSize.
...
func (rg *registry) checkSize(requiredSize int) { // +private-start
	if requiredSize > cap(rg.array) {
		rg.resize(requiredSize)
	}
} // +private-end

func (rg *registry) resize(requiredSize int) { // +private-start
...
func (rg *registry) Push(v LValue) {
	newSize := rg.top + 1
	// +private-call rg.checkSize newSize
	rg.array[rg.top] = v
	rg.top++
}
...
func (rg *registry) CopyRange(regv, start, limit, n int) { // +private-start
	newSize := regv + n
	// +private-call rg.checkSize newSize
...
func (rg *registry) FillNil(regm, n int) { // +private-start
	newSize := regm + n
	// +private-call rg.checkSize newSize
	for i := 0; i < n; i++ {
...
func (ls *LState) closeAllUpvalues() { // +private-start
...
func (ls *LState) rkValue(idx int) LValue {
	/*
		...
		return ls.reg.GetCodeVM(ls.currentFrame.LocalBase + idx)
	*/
	if (idx & opBitRk) != 0 {
...
func (ls *LState) closeUpvalues(idx int) { // +private-start
	if ls.uvcache != nil {
		var prev *Upvalue
...
func (ls *LState) initCallFrame(cf *callFrame) { // +private-start
	if cf.Fn.IsG {
		...
		// +private-call ls.reg.checkSize newSize
		for i := nargs; i < np; i++ {
		...
		if (proto.IsVarArg & VarArgIsVarArg) == 0 {
			...
			*/
			...
			for i := 0; i < np; i++ {
				//ls.reg.V(cf.LocalBase+nargs+i, ls.reg.GetCodeVM(cf.LocalBase+i))
				ls.reg.array[cf.LocalBase+nargs+i] = ls.reg.array[cf.LocalBase+i]
				//ls.reg.V(cf.LocalBase+i, LNil)
				ls.reg.array[cf.LocalBase+i] = LNil
			}
			...
			if CompatVarArg {
				...
				if (proto.IsVarArg & VarArgNeedsArg) != 0 {
					...
					//ls.reg.V(cf.LocalBase+nargs+np, argtb)
...
func (ls *LState) pushCallFrame(cf callFrame, fn LValue, meta bool) { // +private-start
	if meta {
		cf.NArgs++
	...
	ls.stack.Push(cf)
	newcf := ls.stack.Last()
	// +private-call ls.initCallFrame newcf
...
// NewThread returns a new LState that shares with the original co all global objects.
// If the original co has context.Context, the new co has a new child context of the original co and this function returns its cancel function.
...
func (ls *LState) NewProcData(ud interface{}) *ProcData {
	return &ProcData{value: ud}
}

func (ls *LState) NewUserDataByInterface(v interface{}, name string) *LUserData { //key: metatable key
	ud := ls.NewUserData()
	ud.Data = v
...
func (ls *LState) ToLightUserData(n int) *ProcData {
	if lv, ok := ls.Get(n).(*ProcData); ok {
		return lv
	}
	return nil
}
...
// V maximum memory size. This function can only be called from the main thread.
...
// SetContext set a context context to this LState. The provided context must be non-nil.
