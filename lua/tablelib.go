...
func tableNew(L *LState) int {
	ncap := L.CheckNumber(1)
	hash := L.CheckNumber(2)
	L.Push(L.CreateTable(int(ncap), int(hash)))
	return 1
}

func tableSlice(L *LState) int {
	ncap := L.CheckIntOrDefault(1 , 0)
	L.Push(NewSlice(ncap))
	return 1
}

func tableMap(L *LState) int {
	ncap := L.CheckIntOrDefault(1 , 0)
	L.Push(NewMap(ncap , L.IsTrue(2)))
	return 1
}
