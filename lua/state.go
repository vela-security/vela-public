...
//
//func State() *LState {
//	return statePool.Get().(*LState)
//}
//
//func Clone(co *LState) *LState {
//	co2 := State()
//	co2.Code = co.Code
//	co2.B = co.B
//	co2.C = co.C
...
func (ls *LState) PushAny(v any) {
	...
	case LValue:
		ls.reg.Push(value)
	case string:
		ls.reg.Push(S2L(value))
	case int:
		ls.reg.Push(LInt(value))
	case int8:
		ls.reg.Push(LInt(value))
	case int16:
		ls.reg.Push(LInt(value))
	case int32:
		ls.reg.Push(LNumber(value))
	case int64:
		ls.reg.Push(LNumber(value))
	case uint8:
		ls.reg.Push(LInt(value))
	...
	case error:
		...
		ls.reg.Push(S2L(value.Error()))

	case func():
		ls.reg.Push(ls.NewFunction(func(_ *LState) int {
			value()
			return 0
		}))

	case func() interface{}:
		ls.reg.Push(ls.NewFunction(func(co *LState) int {
			co.PushAny(value())
			return 1
		}))

	case LGFunction:
		ls.reg.Push(ls.NewFunction(value))

	case interface{ ToLValue() LValue }:
		ls.reg.Push(value.ToLValue())

	default:
		ls.reg.Push(NewAnyData(v))
	}
...
func (ls *LState) Pushf(format string, v ...interface{}) {
	ls.reg.Push(LString(fmt.Sprintf(format, v...)))
}
...
func (ls *LState) NewProcData(pd LFace) *ProcData {
	return &ProcData{Data: pd}
}
...
func (ls *LState) Current() *callFrame {
	return ls.currentFrame
}
