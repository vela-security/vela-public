...
func copyReturnValues(L *LState, regv, start, n, b int) { // +private-start
	if b == 1 {
		// +private-call L.reg.FillNil  regv n
	} else {
		// +private-call L.reg.CopyRange regv start -1 n
		if b > 1 && n > (b-1) {
			// +private-call L.reg.FillNil  regv+b-1 n-(b-1)
		}
	}
} // +private-end
...
func callGFunction(L *LState, tailcall bool) bool {
	...
	// +private-call L.reg.CopyRange frame.ReturnBase L.reg.Top()-gfnret -1 wantret
	L.stack.Pop()
	L.currentFrame = L.stack.Last()
	return false
}
...
func init() {
	jumpTable = [opCodeMax + 1]instFunc{
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_GETGLOBAL
			...
			RA := lbase + A
			Bx := int(inst & 0x3ffff) //GETBX
			//reg.V(RA, L.getField(cf.Fn.Env, cf.Fn.Proto.Constants[Bx]))
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_SETGLOBAL
			...
			RA := lbase + A
			Bx := int(inst & 0x3ffff) //GETBX
			//L.setField(cf.Fn.Env, cf.Fn.Proto.Constants[Bx], reg.GetCodeVM(RA))
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CALL
			...
			// +private-call L.pushCallFrame callFrame{Fn:callable,Pc:0,Base:RA,LocalBase:RA+1,ReturnBase:RA,NArgs:nargs,NRet:nret,Parent:cf,TailCall:0} lv metaFunc
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_TAILCALL
			...
			// +private-call L.closeUpvalues lbase
			if callable.IsG {
				luaframe := cf
				...
				// +private-call L.initCallFrame cf
				// +private-call L.reg.CopyRange base RA -1 reg.Top()-RA-1
				cf.Base = base
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_RETURN
			...
			if L.Parent != nil && L.stack.Sp() == 1 {
				// +private-call copyReturnValues L reg.Top() RA n B
				switchToParentThread(L, n, false, true)
				return 1
			}
			islast := baseframe == L.stack.Pop() || L.stack.IsEmpty()
			// +private-call copyReturnValues L cf.ReturnBase RA n B
			L.currentFrame = L.stack.Last()
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_CLOSE
			...
			A := int(inst>>18) & 0xff //GETA
			RA := lbase + A
			// +private-call L.closeUpvalues RA
			return 0
		},
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_VARARG
			...
			// +private-call reg.CopyRange RA cf.Base+nparams+1 cf.LocalBase nwant
			return 0
		},
