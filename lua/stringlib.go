...
import (
	...
	"github.com/vela-security/vela-public/lua/pm"
)
...
func strGmatchIter(L *LState) int {
	md := L.CheckUserData(1).Data.(*strMatchData)
	str := md.str
	matches := md.matches
	idx := md.pos
	md.pos += 1
...
func strGmatch(L *LState) int {
	...
	ud := L.NewUserData()
	ud.Data = &strMatchData{str, 0, mds}
	L.Push(ud)
	return 2
}
...
//"suffix":  strSuffix,
//"prefix":  strPrefix,
//"trim_suffix":  strSuffix,
//"trim_prefix":  strPrefix,
//

func strGrep(L *LState) int {
	str := L.CheckString(1)
	pat := L.CheckString(2)
	sep := L.IsString(3)

	g, err := grep.Compile(pat, S2B(sep))
	if err != nil {
		L.PushAny(false)
	} else {
		L.PushAny(g.Match(str))
	}
	return 1
}
...
func strSuffix(L *LState) int {
	str := L.CheckString(1)
	val := L.CheckString(2)
	L.Push(LBool(strings.HasSuffix(str, val)))
	return 1
}
func strPrefix(L *LState) int {
	str := L.CheckString(1)
	val := L.CheckString(2)
	L.Push(LBool(strings.HasPrefix(str, val)))
	return 1
}
...
func strTrimSuffix(L *LState) int {
	...
	if nl > vl && str[nl-vl:] == val {
		L.Push(S2L(str[:nl-vl]))
	} else {
		L.Push(LSNull)
	}
	return 1
