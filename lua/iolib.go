...
func fileLinesIter(L *LState) int {
	...
	if ud, ok := L.Get(1).(*LUserData); ok {
		...
		file = L.Get(UpvalueIndex(2)).(*LUserData).Data.(*lFile)
	}
...
func ioClose(L *LState) int {
	if L.GetTop() == 0 {
		return fileCloseAux(L, fileDefOut(L).Data.(*lFile))
	}
	return fileClose(L)
}

func ioFlush(L *LState) int {
	return fileFlushAux(L, fileDefOut(L).Data.(*lFile))
}
...
func ioRead(L *LState) int {
	return fileReadAux(L, fileDefIn(L).Data.(*lFile), 1)
}
...
func ioWrite(L *LState) int {
	return fileWriteAux(L, fileDefOut(L).Data.(*lFile), 1)
}
