...
// ToLValue creates and returns a new lua.LValue for the given value. Values are
...
// *lua.LUserData with its Data field set to value. The userdata's metatable
...
//
// Struct field accessibility can be changed by setting the field's xreflect tag.
...
// *lua.LUserData with its Data field set to value and no custom metatable.
//

var (
	invalidStructPtr = errors.New("invalid type , must be sturct ptr")
)

func convertObject(key, typ string, filed reflect.Value, v reflect.Value) error {
	if typ != "object" {
		return errors.New(key + " must be " + typ + "got " + v.Type().String())
	}
...
func convert(tag string, typ string, field reflect.Value, tab *lua.LTable) error {
	mTag := strings.Split(tag, ",")
	var key string
	var dv string
	...
	case lua.LTNil:
		switch typ {
		case "int":
			field.Set(reflect.ValueOf(cast.ToInt(dv)))
		case "int8":
			field.Set(reflect.ValueOf(cast.ToInt8(dv)))
		case "int32":
			field.Set(reflect.ValueOf(cast.ToInt32(dv)))
		case "int64":
			field.Set(reflect.ValueOf(cast.ToInt64(dv)))
		case "uint":
			field.Set(reflect.ValueOf(cast.ToUint(dv)))
		case "uint8":
			field.Set(reflect.ValueOf(cast.ToUint8(dv)))
		case "uint32":
			field.Set(reflect.ValueOf(cast.ToInt32(dv)))
		case "uint64":
			field.Set(reflect.ValueOf(cast.ToInt64(dv)))
		case "bool":
			field.Set(reflect.ValueOf(cast.ToBool(dv)))
		case "string":
			field.SetString(dv)

		case "object", "*lua.LFunction", "function":
			field.Set(reflect.ValueOf(nil))

		case "time.Duration":
			if tv, e := time.ParseDuration(dv); e != nil {
				return errors.New(key + "must be time.Duration " + e.Error())
			} else {
	...
	case lua.LTFunction:
		...
		default:
			return errors.New(key + "must be " + typ + " got *lua.LFunction")
		}
	case lua.LTNumber:
		v := lv.(lua.LNumber)
		switch typ {
		...
		default:
			return errors.New(key + "must be " + typ + " got number")
		}
	...
	case lua.LTString:
		...
		case "string":
			field.SetString(v)
		case "time.Duration":
			if tv, e := time.ParseDuration(v); e != nil {
				return errors.New(key + "must be time.Duration " + e.Error())
			} else {
				field.Set(reflect.ValueOf(tv))
			}
		...
		default:
			return errors.New(key + " must be " + typ + " got string")
		}

	case lua.LTBool:
		if typ == "bool" {
			field.SetBool(bool(lv.(lua.LBool)))
		} else {
			field.SetBool(false)
		}

	case lua.LTProcData:
		return convertObject(key, typ, field, reflect.ValueOf(lv.(*lua.ProcData).Data))
	case lua.LTAnyData:
		return convertObject(key, typ, field, reflect.ValueOf(lv.(*lua.AnyData).Data))
	case lua.LTUserData:
		return convertObject(key, typ, field, reflect.ValueOf(lv.(*lua.LUserData).Data))

	default:
		return errors.New(key + " must be " + typ + " got " + lv.Type().String())
	}

	return nil
}

func toArr(L *lua.LState, rv reflect.Value) *lua.LTable {
	if rv.IsNil() {
...
func ToStruct(tab *lua.LTable, obj interface{}) error {
	rt := reflect.TypeOf(obj)
	if rt.Kind() != reflect.Ptr {
		return invalidStructPtr
	}
	rt = rt.Elem()
	...
	for i := 0; i < n; i++ {
		...
		typ, ok := field.Tag.Lookup("type")
		if !ok {
			typ = field.Type.String()
		}

		err := convert(tag, typ, rv.Field(i), tab)
		if err != nil {
			return err
...
back: //循环取地址
	...
	default:
		ud := L.NewAnyData(value, lua.Reflect(lua.ALL))
		return ud
	}
}
...
func (c conversionError) Error() string {
	...
	return fmt.Sprintf("cannot use %v (type %TypeOf) as type %s", val, val, c.Hint)
}
...
func objectToReflectInner(converted lua.LValue, val reflect.Value, hint reflect.Type, tryConvertPtr *bool) (reflect.Value, error) {
...
func lValueToReflectInner(L *lua.LState, v lua.LValue, hint reflect.Type, visited map[*lua.LTable]reflect.Value, tryConvertPtr *bool) (reflect.Value, error) {
		...
		fn := func(args []reflect.Value) []reflect.Value {
			...
			for i, arg := range args {
				if i+1 == len(args) && hint.IsVariadic() {
					...
					for j := 0; j < arg.Len(); j++ {
						arg := arg.Index(j)
						thread.Push(ToLValue(arg.Interface(), thread))
						argCount++
					}
					break
				}
	...
	case *lua.LUserData:
		val := reflect.ValueOf(converted.Data)
		return objectToReflectInner(converted, val, hint, tryConvertPtr)
