...
import (
	"context"
	"errors"
	buffer2 "github.com/vela-security/vela-public/buffer"
	"io"
)
...
func copyBuffer(ctx context.Context, dst io.Writer, src io.Reader, buf *buffer2.Byte) (written int64, err error) {
...
func Copy(ctx context.Context, dst io.Writer, src io.Reader) (written int64, err error) {
	buf := &buffer2.Byte{B: make([]byte, 4096)}
	defer func() {
		buffer2.Put(buf)
	}()
